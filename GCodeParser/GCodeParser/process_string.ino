...
void process_string(char instruction[], int size)
{
  ...
  fp.b = 0.0;
    ...
    has_command('B', instruction, size))
	)
	{
		//which one?
		...
		switch (code)
		{
          ...
          fp.b = search_string('B', instruction, size) + current_units.b;				
				}
		...
		switch (code)
		{
			...
			case 1:
				//set our target.
				set_target(fp.x, fp.y, fp.z, fp.b);
			...
			case 3:
				...
				for (s = 1; s <= steps; s++) {
					...
					set_target(newPoint.x, newPoint.y, fp.z, fp.b);
      ...
      case 5: 
        ...
        fp.b = search_string('B', instruction, size);
            ...
            fp.b = current_units.b;	
					...
					set_target(current_units.x + fp.x, current_units.y + fp.y, current_units.z + fp.z, current_units.b + fp.b);
...
*/
        ...
        if (has_command('M', instruction, size))
	{
		...
		Y_STEP_PIN = search_string('Y', instruction, size);
            ...
            if (has_command('Z', instruction, size)){
	 Z_STEP_PIN = search_string('Z', instruction, size);
                pinMode(Z_STEP_PIN,OUTPUT);  
                digitalWrite(Z_STEP_PIN,LOW);   
            }
             if (has_command('B', instruction, size)){
    B_STEP_PIN = search_string('B', instruction, size);
		...
		Z_DIR_PIN = search_string('Z', instruction, size);
            ...
            if (has_command('B', instruction, size)){
    B_DIR_PIN = search_string('B', instruction, size);
		...
		Z_MIN_PIN = search_string('Z', instruction, size);
            ...
            if (has_command('B', instruction, size)){
    B_MIN_PIN = search_string('B', instruction, size);
		...
		Z_MAX_PIN = search_string('Z', instruction, size);
            ...
            if (has_command('B', instruction, size)){
    B_MAX_PIN = search_string('B', instruction, size);
		...
		Z_STEPS_PER_MM = search_string('Z', instruction, size);
            ...
            if (has_command('B', instruction, size)){
    B_STEPS_PER_MM = search_string('B', instruction, size);
                b_units = B_STEPS_PER_MM;
            }
            break;
            case 7:
		...
		FAST_Z_FEEDRATE = search_string('Z', instruction, size);
            ...
            if (has_command('B', instruction, size)){
    FAST_B_FEEDRATE = search_string('B', instruction, size);
            }
            break;
            case 8:
		...
		SENSORS_INVERTING = search_string('S', instruction, size);
           ...
           case 9:
            ...
            break;
            case 11:
      move_reel(SERVO_1);        // Moves Servo 1
      
      break;
      
            case 12:
      move_reel(SERVO_2);              // Moves Servo 2
      break;

            case 13:
       move_reel(SERVO_3);          // Moves Servo 3
      break;
      
            case 14:
        move_reel(SERVO_4);                // Moves Servo 4
      break;
